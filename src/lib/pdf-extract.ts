...
export async function extractFromPdf(file: File): Promise<ExtractedData> {
  ...
  // V/N.º Contrib.
  const vnContribMatch = fullText.match(
    /V\/N\.?\s*º?\s*Contrib\.?\s*[:\s]*(\d{6,})/i
  );
  ...
  // Empresa
  let empresa = "";
  const empresaMatch = fullText.match(
    /J\.\s*PRUD[EÊ]NCIO[,\s]*LDA/i
  );
  if (empresaMatch) empresa = "J. PRUDÊNCIO, LDA";

  // Contribuinte emissor
  const contribMatch = fullText.match(
    /Contribuinte\s*N\.?\s*º?\s*[:\s]*(\d{9})/i
  );
  const contribuinte = contribMatch ? contribMatch[1] : "";

  // Morada emissor
  let morada_emissor = "";
  const moradaMatch = fullText.match(
    /Parque\s+Industrial\s+de\s+Sete\s+Fontes/i
  );
  if (moradaMatch) {
  ...
  if (emailMatch)
    contactos += (contactos ? " / " : "") + emailMatch[1];

  // Capital Social
  let capital_social = "";
  const capitalMatch = fullText.match(
    /Capital\s+Social\s+([\d\s.,]+\s*EUR)/i
  );
  ...
  if (!dest_nome) {
    const condMatch = fullText.match(
      /(Condom[ií]nio\s+[A-Za-zÀ-ÿ\s,.\-0-9]+?\d+)/i
    );
  ...
  const cargaMatch =
    ...
    fullText.match(
      /Carga[\s\S]{0,80}?(\d{4}-\d{2}-\d{2})(?:\s*\/\s*(\d{1,2}:\d{2}))?/i
    );
  ...
  if (headerIdx >= 0) {
    ...
    for (let i = headerIdx + 1; i < sortedLines.length; i++) {
      ...
      if (
        lower.includes("este documento") ||
        lower.includes("processado por") ||
  ...
  if (items.length === 0) {
    const lineRegex =
      /(\d{6,})\s+([A-Za-zÀ-ÿ0-9.,\-/()\s]+?)\s+(\d{1,3}(?:[.,]\d{1,3})?)\s+(M2|M3|ML|UN|KG|LT|L|PC|CX|SC|UND)\b/gi;
