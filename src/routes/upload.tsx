...
const N8N_WEBHOOK = "https://august-resupply-aflutter.ngrok-free.dev/webhook-test/801d4cc9-1387-4452-95dd-893db9419a7a";
...
function UploadPage() {
  ...
  async function analyze() {
    ...
    setProgress(10);
    try {
      fetch(`${N8N_WEBHOOK}?filename=${encodeURIComponent(file.name)}&size=${file.size}`, { method: "GET", mode: "no-cors" }).catch(() => {});
      setProgress(30);
      ...
      if (data.chave_at_needs_validation) {
        toast.warning("⚠️ Chave AT necessita validação manual!");
      }
      if (data.items.length === 0) {
        toast.warning("Nenhum artigo detetado.");
      } else {
  ...
  const totalQty = items.reduce((s, i) => s + parseFloat(i.quantidade.replace(",", ".") || "0"), 0).toFixed(2);
      ...
      <section className="px-5 mt-5 space-y-4">
        ...
        <label className="block bg-card border-2 border-dashed border-primary/20 rounded-2xl p-7 text-center cursor-pointer hover:border-secondary/50 transition-all hover:shadow-md animate-fade-in-up">
          <input type="file" accept="application/pdf" className="hidden" onChange={(e) => {
            setFile(e.target.files?.[0] ?? null); setItems([]); setChave(""); setExtracted(null); setProgress(0);
          }} />
          ...
          <p className="text-xs text-muted-foreground mt-1">{file ? `${(file.size / 1024).toFixed(0)} KB` : "Toque para escolher"}</p>
        </label>
        ...
        <button disabled={!file || analyzing} onClick={analyze}
          ...
          {analyzing ? (<><Loader2 className="size-5 animate-spin" /> A analisar com OCR avançado...</>) : (<><Sparkles className="size-5" /> Analisar PDF</>)}
        </button>
          ...
          <div className="space-y-4 animate-fade-in-up">
            ...
            {(extracted.qr_at_code || extracted.qr_raw) && (
              <div className="bg-secondary/10 border border-secondary/30 rounded-xl p-4 space-y-2">
                ...
                {extracted.qr_at_code && (
                  <p className="text-xs text-muted-foreground">AT Code: <span className="font-mono font-semibold text-foreground">{extracted.qr_at_code}</span></p>
                )}
                {extracted.atcud && (
                  <p className="text-xs text-muted-foreground">ATCUD: <span className="font-mono font-semibold text-foreground">{extracted.atcud}</span></p>
                )}
              </div>
            )}
            ...
            {extracted.validations.length > 0 && (
              <div className="bg-card rounded-2xl border overflow-hidden">
                <button type="button" onClick={() => setShowValidation(!showValidation)} className="w-full flex items-center gap-2 p-4 text-left hover:bg-muted/30 transition">
                  <ShieldCheck className="size-5 text-secondary" />
                  <span className="flex-1 font-bold text-sm uppercase tracking-wider">Validação ({extracted.validations.length})</span>
            ...
            <div className="bg-card rounded-2xl border p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  <input value={chave} onChange={(e) => setChave(e.target.value)} placeholder="Ex: 19034301713"
                    className={`mt-1 h-11 w-full rounded-lg border px-3 font-mono text-sm outline-none transition ${extracted.chave_at_needs_validation ? "border-amber-400 bg-amber-50 focus:ring-amber-300" : "border-input bg-background focus:ring-ring"} focus:ring-2`} />
                </div>
                <div>
                  <label className="text-xs uppercase tracking-wider text-muted-foreground">Nº Guia</label>
                  <input value={numeroGuia} onChange={(e) => setNumeroGuia(e.target.value)} className="mt-1 h-11 w-full rounded-lg border border-input bg-background px-3 text-sm" />
                </div>
                <div>
                  <label className="text-xs uppercase tracking-wider text-muted-foreground">ATCUD</label>
                  <input value={extracted.atcud} readOnly className="mt-1 h-11 w-full rounded-lg border border-input bg-muted px-3 text-sm font-mono" />
                </div>
                <div>
                  <label className="text-xs uppercase tracking-wider text-muted-foreground">Data Documento</label>
                  <input type="date" value={dataDoc} onChange={(e) => setDataDoc(e.target.value)} className="mt-1 h-11 w-full rounded-lg border border-input bg-background px-3 text-sm" />
                </div>
                <div>
                  <label className="text-xs uppercase tracking-wider text-muted-foreground">V/N.º Contrib.</label>
                  <input value={extracted.vn_contrib} readOnly className="mt-1 h-11 w-full rounded-lg border border-input bg-muted px-3 text-sm font-mono" />
                </div>
              </div>
            </div>

            {/* Emissor */}
            <Collapsible icon={<Building2 className="size-5 text-secondary" />} title="Dados do Emissor / Fornecedor" open={showEmissor} toggle={() => setShowEmissor(!showEmissor)}>
            ...
            <div className="bg-card rounded-2xl border p-4 space-y-3">
              ...
              <ul className="divide-y rounded-xl border overflow-hidden">
                {items.map((it, i) => (
                  <li key={i} className="p-3 flex gap-3 items-start hover:bg-muted/30 transition">
                    <div className="shrink-0 text-center">
                      <span className="text-xs font-mono text-muted-foreground bg-muted rounded-md px-2 py-1 block">{it.artigo}</span>
                    </div>
                ...
                {items.length === 0 && <li className="p-4 text-sm text-muted-foreground text-center">Sem artigos</li>}
              </ul>
            </div>
            ...
            <div className="bg-card rounded-2xl border p-4 space-y-2">
              <label className="text-xs uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                <span className="size-2 bg-secondary rounded-full" /> Observações do Renato
              </label>
              <textarea value={obs} onChange={(e) => setObs(e.target.value)} rows={3} placeholder="Notas..."
      ...
      {(chave || items.length > 0) && extracted && (
        <div className="fixed bottom-0 inset-x-0 bg-background/95 backdrop-blur-lg border-t shadow-2xl p-4 space-y-3 z-50">
          ...
          <div className="grid grid-cols-3 gap-3">
            <button onClick={() => { exportChecklistToExcel(buildChecklist()); toast.success("Excel exportado!"); }}
            ...
            <button onClick={() => shareViaWhatsApp(buildChecklist())}
...
function Collapsible({ icon, title, open, toggle, children }: {
